...
bool FCombatFriendlyFireFilterTest::RunTest(const FString& Parameters)
{
	...
	AGP_PlayerCharacter* PlayerTarget = TestWorld->SpawnActor<AGP_PlayerCharacter>(FVector(200.0f, 0.0f, 0.0f), FRotator::ZeroRotator, SpawnParameters);
	...
	TestTrue(TEXT("Player can still apply combat effects to enemies"), UGP_BlueprintLibrary::CanApplyCombatEffect(PlayerTarget, EnemyTarget));
