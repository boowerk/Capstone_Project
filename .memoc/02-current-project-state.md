...
Last synced: 2026-06-03T04:48:00+09:00
...
- Primary now uses convention-named recovery montages on no-input combo end: after `ActionEnd`, if no next attack is queued, `UGP_Primary` tries target `<attack>_Rec` or the `SwordLight_C_Rec` naming variant before ending. Target attacks no longer fall back into source `_Rec` recovery when target recovery assets are missing, avoiding a target-slot to source-fallback twitch. MaskMan Light A-D montage blend was tuned to `BlendIn=0.12`, `BlendOut=0.20`.
