...
Last synced: 2026-06-04T03:25:00+09:00

## Current Status

- `UGP_RootMotionExtractionEditorLibrary` exposes editor BP-callable helpers for asset-action workflows: duplicate selected `UAnimSequence` assets with a suffix, move pelvis/default source-bone horizontal motion and optional yaw onto the `root` track, optionally zero the source bone XY, and enable root motion on the result. Directional dodge helpers duplicate with `_DodgeRM`, detect the first non-zero XY movement direction, and accumulate absolute projected per-frame speed along that one direction only.
- Primary melee now expects recovery sequences to be embedded inside the attack montages. `UGP_Primary` no longer resolves or plays separate `_Rec` montages and no longer uses recovery/hold/blend timers. `ActionEnd` only opens queued-combo branching; no input lets the current montage continue through embedded recovery and finish normally. Source fallback completion timer uses full montage duration. No UBT/build while editor is open.
- To avoid A-pose/default-slot flashes after natural primary completion, `UGP_Primary::EndAbility` now clears source fallback state without stopping montages when `bWasCancelled=false`; only cancelled endings call `StopUEFNSourceFallbackMontage`. `AGP_PlayerCharacter::StopUEFNSourceFallbackMontage` now calls `Montage_Stop(nullptr)` only when no active fallback montage is known.
- Primary melee disables sprinting while active. `AGP_PlayerCharacter` tracks `bPrimaryAttackInProgress`; `UGP_Primary` sets it on start/end, entering Primary cancels current Sprint, `ToggleSprinting`/`StartSprinting` and `UGP_Sprint::ActivateAbility` refuse activation during Primary, and controller/character speed resolution ignores sprint while Primary is active.
- For hold-sprint controls, `AGP_PlayerController` tracks `bSprintInputHeld`; when Primary ends and the sprint key is still held, `AGP_PlayerCharacter::SetPrimaryAttackInProgress(false)` restarts Sprint automatically. Toggle-sprint mode does not auto-resume from held input.
- Primary movement debugging logs now exist for moving attacks: `[PrimaryMove][Start]` at combo start, `[PrimaryMove]` every ~0.1s while primary movement assist is active, and `[PrimaryMove][Assist]` when assist applies movement. Logs include MovementMode/Falling/ground/floor, velocity, Speed2D, VelZ, acceleration, MaxWalkSpeed, input age, assist ratio, and assist delta speed.
- `CHT_MM_MaskMan_Root_OriginalStyle` now has Crouch rows/subtables added by the user: root routes Standing/Crouching by `Stance`, `MovementMode`, `MovementState`, `Gait`; `Crouch Idles` chooses Dense crouch TIP vs Idle by `ShouldTurnInPlace`; `Crouch Walks` chooses Dense crouch Pivot/Start/Stop/Loop by `IsPivoting`, `IsStarting`, `IsStopping`. PIE validation still pending.
- Hold crouch input exists for testing the new crouch chooser: `IA_Crouch` is mapped to `C` in `IMC_Movement`, `AGP_PlayerController` binds `CrouchAction` when assigned or raw `C` as fallback, and `AGP_PlayerCharacter` enables `NavAgentProps.bCanCrouch`.
- Removed unused motion-matching LOD compatibility variables from `UGP_CharacterAnimInstance`: `EMMDatabaseLOD`, `MMDatabaseLOD`, `MMDatabaseLODEnum`, and the NativeUpdate assignment. Source and narrowed binary Content search found no remaining references outside memoc notes before removal.
- Live Coding remains enabled, but local project user settings set `bPreloadProjectModules=True` and `bPreloadProjectPluginModules=False` so `Project_Eden` stays eligible for Live Coding while `McpAutomationBridge` should be lazy-loaded instead of rebuilt with Live Coding. `Project_EdenEditor Win64 Development` built successfully through UBT after closing the editor.
- The project-local duplicated Interchange parent material attempt for `colormap1`/`Bark_NormalTree_020` was reverted after a D3D12/Nanite render crash during validation. Both uassets were restored from their HEAD LFS object blobs; material usage warnings still need a safer team-persistent fix.
- Primary melee attack targeting uses a short-lived lock-on style flow only inside `UGP_Primary`: `StartComboSequence()` calls `AGP_PlayerCharacter::AimPrimaryAttackAtBestTarget(...)`, which selects a nearby combat-valid pawn, stores it in `TargetActor`, rotates the player toward it during the swing, and interpolates local camera control rotation toward the target. `bIsLockOn` stays false, so constant facing/movement lock-on behavior is not restored; non-primary sword skills remain direction-targeted.
- Primary movement policy is now combo-indexed. `UGP_Primary` no longer grants the global `Fixed` tag. Combo indices 0-1 skip `BeginActionMotionTracking()` so running velocity is not zeroed, request `SetActionLowerBodyMotionMatchBlendEnabled(true)`, and enable weak capsule movement assist (`MobileAttackMovementAssistSpeedRatio=0.35`) while movement input is held. MaskMan `MaskMan_Sword_Light_A/B` AnimSequences have root motion disabled; C/D keep root motion for heavier attacks. Combo indices 2+ keep action root-motion tracking/carry.
- Primary melee SourceDodge/direct-dodge experiment has been removed from C++ execution paths. `UGP_Primary` no longer marks a melee-dodge state or buffers Alt dodge, `AGP_PlayerController::Input_Dash` no longer has a Primary special case/direct LeftAlt fallback, and `UGP_Dash` uses Roll montages only.
- Normal Roll direction is constrained to a control-yaw 60-degree forward cone: `UGP_Dash` allows F/FL/FR movement input when choosing roll-facing direction even while walking, but L/R/B/BL/BR inputs cancel before `CommitAbility` so they do not roll or spend cooldown/cost.
...
- Skill visual data is no longer only fixed actor/projectile VFX. `UGP_SkillData.VisualCues` stores per-skill visual entries keyed by optional cue/situation tag, optional element tag, and visual type (`Actor` or `Niagara`); `UGP_SkillBase` resolves the most specific match first and falls back to legacy `SkillVisualActorClass` / `ElementVisualActorClasses`.
- Action root-motion movement is componentized as `authored/retargeted RM + shared carry`: target montages use their authored RM, source fallback montages use runtime-retarget consumed RM multiplied by `MovementSpeedScaleRatio`, and both add the same entry-velocity carry path. `BeginActionMotionTracking()` captures entry XY velocity then zeros CMC XY to avoid fallback double-consuming old movement velocity. Handoff applies remaining carry velocity only, not sampled RM+carry.
- UEFNSource fallback montage debug/anim speed reads `AGP_PlayerCharacter::GetCurrentActionMotionVelocity()` during fallback playback because manual `SafeMoveUpdatedComponent` root motion is not reflected in `CharacterMovement->Velocity`.
- Current roll RM source assets differ: `/Game/Characters/MaskMan/Animations/MaskMan_Roll_RM` root distance is ~593.9, while `/Game/Characters/UEFN_Mannequin/Animations/Montage/Roll/UEFN_Roll_RM` is ~465.5. With MaskMan `MovementSpeedScaleRatio=1.22`, fallback expected distance is ~568 before carry/collision.
- `AGP_PlayerCharacter.FallbackRootMotionDistanceCorrection` defaults to `1.18` to compensate observed runtime fallback loss: user measured target roll ~590-600 and fallback ~502 with only MovementSpeedScaleRatio; corrected expected fallback is ~592.
